...
defmodule CiscoSNMP do
  alias CiscoConfigCopy.CcCopyEntry

  defp get_copy_state(row, agent, credential) do
    result =
      CcCopyEntry.ccCopyState
        |> SNMPMIB.index(row)
  ...
  defp _await_copy_result(row, agent, credential, tries) do
    case get_copy_state row, agent, credential do
      ...
      4 ->
        fail_cause =
          get_copy_fail_cause(row, agent, credential)
  ...
  defp process_copy_entry(copy_entry, agent, credential) do
    row = :random.uniform 60000

    try do
      [{:ok, _}, {:ok, _}|_] =
        set_copy_entry_row copy_entry, row, agent, credential

      [{:ok, _}] = set_copy_entry_row_status copy_entry, row, agent, credential
